...
class TasksHRI:
    ...
    def __init__(self, fake=False) -> None:
        ...
        if not self.FAKE_TASK:
            self.conversation_client = actionlib.SimpleActionClient(
                CONVERSATION_SERVER, ConversateAction)
            self.pub_speak = rospy.Publisher(
                SPEAK_TOPIC_NOW, String, queue_size=10)
            ...
            try:
                ...
                self.items_category_client = rospy.ServiceProxy(
                    ITEMS_CATEGORY_SERVER, ItemsCategory)
            ...
            try:
                ...
                self.object_category_client = rospy.ServiceProxy(
                    OBJECT_CATEGORY_SERVER, ItemsCategory)
            except rospy.ROSException as e:
                rospy.logwarn(
                    "[WARNING] Object category service not available")

        else:
            rospy.loginfo("[INFO] Fake HRI Task Manager initialized")

        rospy.loginfo("[SUCCESS] HRI Task Manager initialized")
        self.keyword_sub = rospy.Subscriber(
            "keyword_detected", Bool, self.callbackKeyword)
    ...
    def analyze_guest(self, guest_id: int) -> str:
        ...
        client = actionlib.SimpleActionClient(
            GUEST_ANALYSIS_SERVER, GuestAnalysisAction)
    ...
    def guest_analysis_done(self, status, result) -> None:
        ...
        rospy.loginfo(
            f"Guest analysis result: {result.description} for guest {result.guest_id}")
