...
class ReceptionistTaskManager:
    ...
    def __init__(self) -> None:
        ...
        self.guests = [
            Guest(0, "John", "water",
                  "Has dark brown hair, and is wearing a blue t-shirt and shorts."),
            Guest(1),
            Guest(2),
        ]
        ...
        while not rospy.is_shutdown():
            ...
            # Self introduction
                ...
                self.subtask_manager["hri"].speak(
                    "Hi, my name is Frida. I'll be your receptionist today. Could you tell me your name and your favorite drink?", wait=True)
            ...
            # Request name and favorite drink and store in current guest object
                ...
                if name != "error":
                    ...
                    self.subtask_manager["hri"].speak(
                        f"Nice to meet you {name}, please stay in front of me while I recognize your face.", wait=True)
                    self.current_state = STATES["SAVE_USER_FACE"]
                else:
                    self.subtask_manager["hri"].speak(
                        "I'm sorry, I didn't get your information.", wait=True)
            ...
            # Store user face with its name and call image analysis
                ...
                if self.follow_face():
                    self.subtask_manager["hri"].analyze_guest(
                        self.current_guest)
                    self.subtask_manager["vision"].save_face_name(
                        self.guests[self.current_guest].name)
                    self.subtask_manager["hri"].speak(
                        "I have saved your face, thank you. Please follow me to the living room.", wait=True)
                    self.current_state = STATES["GO_TO_LIVING_ROOM"]
                else:
                    self.subtask_manager["hri"].speak(
                        "I'm sorry, I couldn't recognize your face. Please stay in front of me.", wait=True)
            ...
            # Navigate to the living room
                ...
                self.subtask_manager["hri"].speak(
                    "The host is already waiting for you there. Please stay behind me until I find your seat.", wait=False)
                self.subtask_manager["manipulation"].move_arm_joints(
                    0, 0, "face_detection")
            ...
            # Introduce guest 1
                ...
                description = self.subtask_manager["hri"].get_guest_description(
                    1)
                ...
                self.subtask_manager["hri"].speak(
                    f"Hi {self.guests[0].name}, this is {self.guests[1].name}. It's favorite drink is {self.guests[1].favorite_drink}", wait=True)
                self.subtask_manager["hri"].speak(
                    f"{self.guests[1].description}", wait=False)
                ...
                for location in face_locations:
                    ...
                    for face in current_faces:
                        ...
                        if self.follow_face() and introduced_to < 2:
                            introduced_to += 1
                            self.subtask_manager["hri"].speak(
                                f"Hi {face.name}, this is {self.guests[2].name}. It's favorite drink is {self.guests[2].favorite_drink}", wait=True)
                            self.subtask_manager["hri"].speak(
                                f"{self.guests[2].description}", wait=False)
            ...
            # Gaze at the current guest and inform next action
                ...
                self.subtask_manager["hri"].speak(
                    f"I'll find you a free seat, {self.guests[self.current_guest].name}, please wait.", wait=False)
            ...
            # Find a free seat for the guest
                ...
                if seat_angle == 300:
                    self.subtask_manager["hri"].speak(
                        "I'm sorry, I couldn't find a free seat for you. Please sit where you prefer", wait=False)
                else:
                    # self.subtask_manager["manipulation"].move_arm_joints(0, 0, "seat")
                    self.subtask_manager["hri"].speak(
                        "I have found a free seat for you, please follow the direction of my arm.", wait=False)
                    self.subtask_manager["manipulation"].move_arm_joints(
                        seat_angle, -20)
            ...
            # Wait for the user to sit
                ...
                self.subtask_manager["hri"].speak(
                    "I've detected you took your seat. I'll go back to the entrance now.", wait=False)
                if self.current_guest < 2:
                    self.current_guest += 1
                    self.current_state = STATES["GO_TO_ENTRANCE"]
                else:
                    self.subtask_manager["hri"].speak(
                        "I have finished my task.", wait=False)
